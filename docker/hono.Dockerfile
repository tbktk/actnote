...
# Honoバックエンドをビルド
RUN pnpm turbo build --filter=api.hono

# Honoバックエンドとその依存関係のみを含むようワークスペースをprune
RUN pnpm turbo prune --scope=api.hono --docker
...
COPY --from=builder /app/out/full/apps/api.hono /app/apps/api.hono
...
CMD ["node", "apps/api.hono/dist/index.js"]